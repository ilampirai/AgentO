...
import { parseFunctions } from '../memory/parser.js';
import type { SymbolInput, FlowGraph, SymbolNode, FunctionEntry, ClassEntry } from '../types.js';
...
export async function handleSymbol(args: unknown) {
  const input = args as SymbolInput;
  ...
  try {
    ...
    for (const result of results) {
      if (result.kind === 'function' || result.kind === 'method') {
        const func = functions.find(f => 
          f.name === result.name && f.file === result.file
        );
        if (func) {
    ...
    for (const result of results) {
      ...
      if (result.details) output += `- Details:\n  ${result.details.split('\n').join('\n  ')}\n`;
      output += '\n';
    }
