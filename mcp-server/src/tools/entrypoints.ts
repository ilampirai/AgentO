...
import type { EntryPointsInput, FlowGraph, SymbolNode } from '../types.js';
...
export async function handleEntryPoints(args: unknown) {
  const input = args as EntryPointsInput;
  ...
  try {
    ...
    const matches: Array<{ id: string; node: SymbolNode; score: number }> = [];
