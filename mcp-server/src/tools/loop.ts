...
import type { LoopInput, LoopState, LoopIteration } from '../types.js';
...
export async function handleLoop(args: unknown) {
  const input = args as LoopInput & { action?: string };
  ...
  const state = await readJsonMemoryFile<LoopState>(MEMORY_FILES.LOOP_STATE, defaultState);
