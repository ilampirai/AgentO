...
import { readJsonMemoryFile, writeJsonMemoryFile, memoryFileExists, readMemoryFile } from '../memory/loader.js';
    ...
    try {
        switch (action) {
            ...
            case 'get': {
                ...
                if (key) {
                    ...
                    return {
                        content: [{
                                type: 'text',
                                text: `⚙️ ${key} = ${JSON.stringify(config[key])}`,
                            }],
                    };
                }
                // Show all config
            ...
            case 'set': {
                ...
                const defaultType = typeof DEFAULT_CONFIG[key];
            ...
            case 'reset': {
                if (key) {
                    ...
                    return {
                        content: [{
                                type: 'text',
                                text: `✅ Reset ${key} to default: ${JSON.stringify(DEFAULT_CONFIG[key])}`,
                            }],
                    };
                }
                // Reset all
...
async function getStatus() {
    ...
    // Loop state
    try {
        const loopState = await readJsonMemoryFile(MEMORY_FILES.LOOP_STATE, {
            active: false,
            task: '',
            completionMarker: '',
            maxIterations: 10,
