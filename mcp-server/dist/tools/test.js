...
export async function handleTest(args) {
    const input = args;
    ...
    try {
        ...
        while (attempt <= maxRetries && !success) {
            ...
            try {
                ...
                success = detectedFramework.successMarkers.some(marker => lowerOutput.includes(marker.toLowerCase()));
                // Also check no failure markers
                const hasFailure = detectedFramework.failureMarkers.some(marker => lowerOutput.includes(marker.toLowerCase()));
                if (hasFailure) {
                    success = false;
                }
            }
