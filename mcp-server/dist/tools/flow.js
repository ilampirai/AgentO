...
export async function handleFlow(args) {
    const input = args;
    const { ids, depth = 2, direction = 'both', maxNodes = 100, maxEdges = 200 } = input;
    try {
        ...
        while (queue.length > 0 && Object.keys(resultNodes).length < maxNodes) {
            ...
            for (const neighborId of neighbors) {
                ...
                if (resultNodes[id] && resultNodes[neighborId] && resultEdges.length < maxEdges) {
                    const edge = flowGraph.edges.find(e => (e.from === id && e.to === neighborId) ||
